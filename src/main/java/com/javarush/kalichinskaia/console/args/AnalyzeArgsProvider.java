...
import static com.javarush.kalichinskaia.console.AppMode.ANALYZE_MODE;

public class AnalyzeArgsProvider implements ArgsProvider {

    @Override
    public List<String> get(Scanner scanner) {
        List<String> args = new ArrayList<>();
        args.add(String.valueOf(ANALYZE_MODE.getMode()));

        System.out.println("Enter source (full path OR only filename) :");
        String answer = scanner.nextLine();
        args.add(answer);

        System.out.println("Enter dictionary  (full path OR only filename) :");
